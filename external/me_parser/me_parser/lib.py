#!/tcghome/fgold/Enthought/Canopy_64bit/User/bin/python
## #!/usr/bin/env python
"""
...
def fit_arrhenius(data, k):
	...
	# more than 3 k's are positive, so fit A, n, Ea	
		...
		X = np.array( [ np.ones( len(local_T) ), np.log(local_T/data.T0), -1.0 / data.R / local_T ],dtype=np.float64 )
		X = X.transpose()
		theta = np.linalg.lstsq(X, np.log(local_k))[0]
		A = np.exp(theta[0])
		n = theta[1]
...
def get_double_arrhenius_OLD(data, k, d_plog, nonlin_fit):
	...
	if d_plog.curvature=='down':
		...
		if nonlin_fit:
#			plsq = leastsq(mod_arr_residuals, p0, args=(local_k, local_T, data), ftol=1.0E-9, xtol=1.0E-9, maxfev=10000  )
			plsq = leastsq(mod_arr_residuals, best_guess, args=(local_k, local_T, data), \
	       Dfun=mod_arr_jacobian_finite_difference,col_deriv=1,factor=0.1,  ftol=1.0E-8, xtol=1.0E-8, maxfev=10000  )
#			coeff, var_matrix = curve_fit(nonlin_wrapper_double(data), local_T, np.log(local_k), p0=p0, ftol=1.0E-9, xtol=1.0E-9, maxfev=10000000)
#			simplex_results = simplex(simplex_residuals, p0, args=(local_k, local_T, data), ftol=1.0E-12, xtol=1.0E-12, maxfev=10000  )
#			plsq = [coeff,0]
		else:	
			plsq = [p0,0]
	...
	# now switch to curves that are convex up, i.e. d^2k/dT^2 >=0		
		...
		for run in range(N_runs):
			...
			X = np.array( [ np.ones( len(new_T) ), np.log(new_T/data.T0), -1.0 / data.R / new_T ],dtype=np.float64 )
			X = X.transpose()
		...
		if nonlin_fit:
#			plsq = leastsq(mod_arr_residuals, p0, args=(local_k, local_T, data), ftol=1.0E-12, xtol=1.0E-12, maxfev=10000  )
			plsq = leastsq(mod_arr_residuals, best_guess, args=(local_k, local_T, data), \
	       Dfun=mod_arr_jacobian_finite_difference,col_deriv=1, ftol=1.0E-12, xtol=1.0E-12, maxfev=10000  )			
#			coeff, var_matrix = curve_fit(nonlin_wrapper_double(data), local_T, np.log(local_k), p0=p0, ftol=1.0E-9, xtol=1.0E-9, maxfev=10000000)
#			plsq = [coeff,0]
		else:	
			plsq = [p0,0]
#	p0 = [s_A*0.4, s_n, s_Ea, s_A*0.6, s_n, s_Ea ]
#	plsq = leastsq(residuals, p0, args=(local_k, local_T, data), ftol=1.0E-12, xtol=1.0E-12, maxfev=10000 )		
	# assign the results		
	d_plog.fit_range = fit_range			# fit range
...
def get_k_curvature(data, T, k):
	""" this subroutine determines whether the rate constant curves up or down."""
...
def simple_arr_residuals(p, target, T, data, n1, n2):
	...
	for t in range( len(T) ):
		# you have a choice of minimizing the difference in the log of k, or minimizing the relative difference.
...
#			if p==int(np.median(range(len(pressure)))):
